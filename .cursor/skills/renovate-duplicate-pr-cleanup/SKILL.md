...
- **Straggler body signal**: a strong quick-filter indicator is the body text *"Automerge: Disabled because a matching PR was automerged previously"*. **However, this signal is necessary but not sufficient** — MintMaker also sets it when a *previous cycle* of the same package update (e.g. an earlier digest bump) was auto-merged on that branch. An open PR with this signal but **no same-branch merged twin for the same digest** is a legitimate pending PR, not a straggler — do **not** close it. Always complete the full merged-twin search (Step 3) before acting on this signal.
- **Mixed-merge batches**: occasionally a human reviewer manually merges some PRs in a cross-branch batch (e.g. merging `rhoai-3.4` and `rhoai-2.25` variants) while leaving others open (e.g. `rhoai-3.3` and `main`). Those remaining open PRs are **not duplicates** — they are legitimate PRs awaiting the same treatment. Check `mergedBy` on any "canonical" merged PR: if it is a human (not the bot), the open counterparts on other branches likely also need manual merging, not closing.
- **Pipelines as Code (PaC) configuration proposals**: alongside digest-bump stragglers, the Konflux bot also opens PRs titled *"Konflux Internal p02 update …"* with body *"# Pipelines as Code configuration proposal"*. These are **default onboarding PRs** created automatically when a new Konflux component is registered. They are **always unwanted** because Tekton pipeline definitions (`.tekton/` directory) already exist on the target branch. The established pattern is to close them without merging, with a comment such as: *"This PR's changes are unwanted — it is a default PR created once the Konflux component is onboarded. We already have Tekton pipeline definitions for `<branch>` in `.tekton/`. Closing."* Link to a previous example (e.g. #1336 on `rhoai-3.5-ea.1`) for context. Historically, every such PR across `rhoai-3.4`, `rhoai-3.4-ea.2`, `rhoai-3.5-ea.1`, `rhoai-3.5-ea.2`, and others has been closed this way.
...
3. **Separate PaC proposals from digest-bump stragglers**: PRs with body containing *"# Pipelines as Code configuration proposal"* (titles like *"Konflux Internal p02 update …"*) are a different category — handle them per the PaC convention above (close with a note about existing `.tekton/` definitions). Do **not** run the merged-twin search workflow (Steps 2–5) on these; they are never duplicates of a merged digest bump.

4. **Quick open-vs-open clustering** (optional shortcut): group open PRs by **`(base branch, normalized title)`** (normalize by stripping trailing ` (main)` / ` (rhoai-…)` if you use that convention). More than one PR in a group ⇒ candidate duplicate cluster. **Do not stop here** — even a **singleton** open PR still needs **Step 3 merged lookup**.
