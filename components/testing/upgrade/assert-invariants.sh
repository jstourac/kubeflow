...
assert_resources_present_for_auth_notebook() {
  ...
  # Historical naming differs across controller versions.
  if ! kubectl -n "${WORKLOAD_NAMESPACE}" get networkpolicy "${AUTH_NOTEBOOK_NAME}-oauth-np" >/dev/null 2>&1; then
    kubectl -n "${WORKLOAD_NAMESPACE}" get networkpolicy "${AUTH_NOTEBOOK_NAME}-kube-rbac-proxy-np" >/dev/null
  fi
