...
func (r *NotebookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if ss.Spec.Replicas != nil && foundStateful.Spec.Replicas != nil && *ss.Spec.Replicas != *foundStateful.Spec.Replicas {
		...
		for k, v := range ss.Spec.Template.ObjectMeta.Labels {
			if foundStateful.Spec.Template.ObjectMeta.Labels == nil || foundStateful.Spec.Template.ObjectMeta.Labels[k] != v {
				needsLabelUpdate = true
				break
			}
		}
		if needsLabelUpdate {
			if foundStateful.Spec.Template.ObjectMeta.Labels == nil {
				foundStateful.Spec.Template.ObjectMeta.Labels = map[string]string{}
			}
			for k, v := range ss.Spec.Template.ObjectMeta.Labels {
				foundStateful.Spec.Template.ObjectMeta.Labels[k] = v
			}
		}
	}
